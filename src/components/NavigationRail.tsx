...
export const NavigationRail: React.FC<NavigationRailProps> = ({
  ...
  const navItems = [
    ...
    },
    {
      ...
      badge:
        exceptionCount > 0 ? (
          <span className="bg-rose-100 text-rose-800 text-[10px] font-bold px-1.5 py-0.5 rounded-full font-mono">
            {exceptionCount}
          </span>
        ) : null,
  ...
  return (
    <>
      ...
      {/* Rail Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-40 bg-slate-900 text-slate-300 flex flex-col transition-all duration-200 ease-in-out border-r border-slate-800 ${
      ...
      >
        ...
        <div className="p-3 border-t border-slate-800 shrink-0">
          {!isCollapsed && (
            <div className="bg-slate-800/80 rounded-xl p-2.5 mb-2 border border-slate-700/50">
          ...
          <button
            ...
            className="hidden lg:flex w-full items-center justify-center p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer"
